...
| `bridge-service-diagnostics.test.mjs` | bridge diagnostics suite | Covers help, status, degraded-session reporting, current attached-thread runtime reporting, running-message previews, changes, and last-error Telegram command behavior. |
...
| `control-server.test.mjs` | control plane suite | Verifies health, status, graceful shutdown, and force-shutdown HTTP responses. |
