...
| `bridge-service.mjs` | runtime orchestrator | Owns the single attached thread session, per-chat relay queues, attached-thread busy/interrupt semantics, degraded/idle/busy/draining mode transitions, and access updates. |
| `codex-app-server.mjs` | Codex transport adapter | Wraps the Codex app-server JSON-RPC protocol used for one attached thread session, active-turn inspection, interruption, and turn relay. |
...
| `bridge-diagnostics.mjs` | diagnostics helper | Builds Telegram-facing help/status/error replies, including session readiness and current attached-thread runtime summaries. |
